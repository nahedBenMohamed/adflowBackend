...
@Controller()
@JwtAuthorized()
export class UnspamMailThreadController {
  constructor(private mailboxService: MailboxService) {}

  @Put('/mailing/mailboxes/:mailboxId/threads/:messageId/unspam')
  public async unspamMailThread(
    @CurrentAuth() { accountId, userId }: AuthData,
    @Param('mailboxId', ParseIntPipe) mailboxId: number,
    @Param('messageId', ParseIntPipe) messageId: number,
  ) {
    return await this.mailboxService.unspamThread(accountId, userId, mailboxId, messageId);
  }
}
